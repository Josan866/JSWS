//setup
const urlcd = window.location.search
const urlqm = new URLSearchParams(urlcd)
let urlq;
//urlq = urlqm.get('code'); checking for search parameter

//
function secretcode(){
urlq = urlqm.get('secret'); 
if (urlq == "true") {
    document.getElementById("ecd").innerHTML = '<form action="./3nte4c0d3s2" method="GET"><label for="k0b3" id="Lable">Enter Code: </label><input type="" id="k0b3" name="code" placeholder="scrt..." /><br><br><input type="submit">';
    urlq = urlqm.get('wc');
    if (urlq == 1) {
        document.getElementById("Lable").innerHTML = "Code is incorrect: ";
    }
    else if (urlq == 2) {
        document.getElementById("Lable").innerHTML = "try again: ";
    }
    else if (urlq == 3) {
        document.getElementById("Lable").innerHTML = "Please try again later: ";
    };
};};

function download(file){
    var link = document.getElementById("dcode2");
    link.setAttribute("href", file);
    link.click();
};

function terni(){  //   ./r3gisred check if terni
urlq = urlqm.get('dog');  // checking for search parameter
if (urlq == 7777666777777999) {
    document.getElementById("dog1").innerHTML = "sorry";
    document.getElementById("dog2").innerHTML = "I´m sorry for your loss and i hope you get over it. You make awesome videos and i hope you continue. Kind reguards Josan866, marketing of J´s Markt Companies";
}};


function hscodeentr(){ //   ./entercode 
    window.location = "./entercode?scrt=728658";
};

//secret-code = <form action="./3nte4c0d3s" method="GET">
